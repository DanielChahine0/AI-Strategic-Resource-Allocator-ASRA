"""Deterministic software-capability matrix.

The simplified allocator's only hard rule is whether a device tier can run the
requested software. This maps a software keyword to the minimum device tier that
can run it (sourced from the shared sample_data/software_capability_matrix.json,
with a hard-coded fallback). The category/tier direct-linking rules were removed
in the Phase-5 simplification.
"""
...
# Single source of truth shared with the RAG engine.
...
def device_meets_software(device_tier: DeviceTier | None, software_needed: list[str]) -> bool:
    """True if the device tier can plausibly run the requested software."""
