...
Everything the engine does with Gemini — each call, its outcome, and every
fall-back decision — is emitted here to **stderr** so it shows up live in the
terminal (and in run.sh's ``.run-logs/ai-model.log``). This is deliberately
separate from the structured JSONL audit log (``logs/llm_audit.jsonl``), which
is for after-the-fact analysis; this stream is for *watching the engine think*
while it runs.
