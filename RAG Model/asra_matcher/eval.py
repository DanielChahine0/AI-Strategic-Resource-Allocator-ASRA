"""Evaluation-run mode for the RAG Model.

Runs the retrieval-augmented matching engine over a labelled dataset and emits,
for every applicant, the four metrics the comparison UI needs:

  * token usage   — Gemini prompt/candidate/total counts across the tier
                    recommendation + explanation calls (0 when the LLM is
                    unavailable and deterministic fallbacks run)
...
The response contract (`EvalResult`) is intentionally **identical** to the AI
Model's, so the frontend can run both and diff them field-for-field. RAG-only
signals (grounding citations, tier-recommendation confidence) ride along in
fields the AI model leaves empty.

`/match` and the engine are untouched — this module only *reads* the pipeline
(plus the additive `chosen_tier_confidence` field already on FinalMatchResult).
"""
...
# ---------------------------------------------------------------------------
# Response contract (shared, byte-for-byte, with the AI Model)
# ---------------------------------------------------------------------------
...
def score_confidence(
    ...
    so the two models' confidence numbers are directly comparable. RAG's own
    tier-recommendation confidence is reported separately, not folded in here.
    """
...
def _usage_keywords(main_usage: str) -> list[str]:
    """RAG stores main_usage as one free-text string; split it into phrases."""
    parts = [p.strip() for p in main_usage.replace(";", ",").split(",")]
    return [p for p in parts if p]


def _evaluate_one(applicant: Applicant, inventory: list[Device], truth: dict) -> EvalRow:
    scenario = truth.get("scenario", applicant.id)
    ...
    try:
        ...
        return EvalRow(applicant_id=applicant.id, scenario=scenario, error=repr(exc))
    finally:
        captured = llm.read_token_capture() or {}
        llm.stop_token_capture()

    top: list[MatchResult] = result.top_matches
    ...
    # Fallback if no Gemini call landed, or the engine flagged a fallback in its notes.
    fallback_used = int(captured.get("calls", 0)) == 0 or any(
        "fell back" in n.lower() or "fall back" in n.lower() for n in result.notes
    )

    chosen_category = result.chosen_application.category.value
    if chosen is None:
        return EvalRow(
            ...
            tokens=tokens,
            tier_recommendation_confidence=result.chosen_tier_confidence,
            fallback_used=fallback_used,
            error="no device passed the fit gate",
        )

    chosen_tier = chosen.device.tier.value if chosen.device.tier else None
    composite = chosen.breakdown.composite
    runner_up_composite = runner_up.breakdown.composite if runner_up else None
    intake = result.chosen_application.intake
    explanation = result.explanations.get(chosen.device.id) or chosen.explanation
    ...
    return EvalRow(
        ...
        tokens=tokens,
        accuracy=score_accuracy(chosen_category, chosen_tier, chosen.device.id, truth),
        ...
        explanation_quality=score_explanation_quality(
            ...
            main_usage=_usage_keywords(intake.main_usage),
            tier=chosen_tier,
        ),
        citations=list(chosen.citations),
        tier_recommendation_confidence=result.chosen_tier_confidence,
        fallback_used=fallback_used,
    )
...
def run_eval(dataset: str = "sample-v1", limit: int | None = None) -> EvalResult:
    ...
    rows = [_evaluate_one(a, inventory, ground_truth.get(a.id, {})) for a in applicants]
